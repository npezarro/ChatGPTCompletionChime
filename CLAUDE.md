...
- **Auto-update headers required:** The `.user.js` file must include `@updateURL` and `@downloadURL` pointing at the hosted copy (e.g., `https://example.com/ChatGPTCompletionChime.user.js`). Without these, Tampermonkey cannot detect updates.
